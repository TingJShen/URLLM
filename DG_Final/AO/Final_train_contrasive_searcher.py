...
def eval(trained_model,eval_length):
    ...
    with open('./test_F2.txt','r') as f:
        lines = f.readlines()
    user_representations_testX= np.load('../AO/DG_390/DGAO_final_test_x_fea.npy')
    user_representations_testY= np.load('../AO/DG_390/DGAO_final_test_y_fea.npy')
    XORY_test= np.load('../AO/XORY_test.npy')
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    tmp_X=np.load('../AO/DG_390/DGAO_final_train_x_fea.npy')
    user_representationsX = np.load('../AO/DG_390/DGAO_final_train_x_fea.npy')
    user_representationsY = np.load('../AO/DG_390/DGAO_final_train_y_fea.npy')
    ...
    XORY=np.load('../AO/XORY_train.npy')
    ...
    np.save('./saver/best_trte_XORY_DG_390_.npy',max_best_trte)
