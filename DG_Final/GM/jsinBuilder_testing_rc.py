...
data = pd.read_csv('./item_listM_F.csv',sep=',',header=0).values[0::,0::]
first_items=[]
for item in data:
    first_items.append(item[1])
split_len=len(first_items)
print(first_items[0],len(first_items))
data = pd.read_csv('./item_listG_AM_F.csv',sep=',',header=0).values[0::,0::]
#first_items=[]
for item in data:
    first_items.append(item[1])
...
for line_idx in tqdm(range(len(lines))):
    line=lines[line_idx]
    tmp_dict={}
    tmp_dict["instruction"]="Given a list of games or movies the user has bought before, please recommend a new video game that the user likes to the user."
    tmp_dict["input"]="The user has played the following video games before:"
    tmp_line=line.split('\t') 
    tmp_end_word=""
    tmp_seq_list=[]
    for case in range(2,len(tmp_line)-1,1):
        ...
        if((tmp_line[case].split('|')[1].replace('\n',''))!=(tmp_line[1])):
            ...
            if case_id>=split_len:
                tmp_dict["input"]=tmp_dict["input"]+" Game:"+first_items[case_id]+' |'
            else:
                tmp_dict["input"]=tmp_dict["input"]+" Movie:"+first_items[case_id]+' |'

        else:
            ground_truth=int(tmp_line[case].split('|')[0].replace('\n',''))
            ground_truths.append(ground_truth)
            if int(tmp_line[case].split('|')[0].replace('\n',''))<split_len: 
                tmp_end_word='Please recommend a new movie\n'
            else:
                tmp_end_word='Please recommend a new game\n'
    ...
    for ii in range(0,1,1):
        tmp_input+='There is another similar user who has played movies and games before:'
        ...
        for item_train in similar_seq[argsort_trte[ii]]: 
            if ((len(tmp_dict["input"].split(' '))+len(tmp_input.split()))<max_tk):
                if item_train < split_len:
                    tmp_input=tmp_input+" Movie:"+str(first_items[item_train])+' |'
                else:
                    tmp_input=tmp_input+" Game:"+str(first_items[item_train])+' |'
